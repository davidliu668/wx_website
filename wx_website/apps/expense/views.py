...
def web_root_index(request):
    logger.info('access the website root index.html')
...
def export_exp(request):
    ...
    for exp in latest_expense_list:
        ...
        exp_dict = {'money': exp.money, 'time': time_str,
                    'is_exp': is_exp, 'pic_url': exp_pic_url}
    ...
    html = html_template.render(
        {'summery': summery, 'no_exp_list': no_exp_list}).encode('utf-8')
...
def detail(request, expense_id):
    ...
    if expense_id == "0":
        expense = expense_list(
            pk=0, money=0, time=datetime.datetime.now(), remark='')
    else:
...
def add(request, expense_id):
    ...
    if input_file is None:
        ...
        cos_client = CosClient(cos_appid, cos_secret_id,
                               cos_secret_key, cos_region)
        cos_upload_req = UploadFileRequest(
            u'expense', u'/' + filename.decode('utf8'), filepath.decode('utf8'))
